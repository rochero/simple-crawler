...
def parse_data(p:HTMLResponse):
    ...
    try:
        ...
        if s:
            ...
            id = r.url.split('/')[-2]
            for x in range(1,11):
                y = j.get('countVote{}'.format(x))
                y = int(y) if y else 0
                score += (y*x)
                vote += y
            score = score/vote if vote else 0
        return [id, score, vote]
    except Exception:
...
def crawl(u_list=None, save_path:str='', chunk_len:int = 0, max_workers:int = 0, lag:float = 0):
    global flag
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        t2.join()
        print('保存中。。。')
        append_data_xlsx(d_list,save_path,'Sheet2')
        system('cls')
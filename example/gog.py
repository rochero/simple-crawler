...
def parse_data(p):
    ...
    score = ''
    cnt = ''
    # id,title,genre,desc,dev,pub = ['']*6
    try:
        if r:
            s = r.html
            j = json.loads(s.html) if s and s.html else ''
            if j:
                score=j.get('value')
                cnt=j.get('count')
                return [score,cnt]
        ...
        #     id = re.search('product_id: \'(\d+?)\',',s.html,re.S)
        #     id = id.group(1) if id else ''
        #     desc = s.xpath('//div[@class="description"]',first=True)
        #     desc = desc.text if desc else ''
        #     genre = s.xpath('//div[@class="details__content table__row-content"]/a[contains(@href,"category")]/text()')
        #     genre = '|'.join(genre) if genre else ''
        #     dev = s.xpath('//div[@class="details__content table__row-content"]/a[contains(@gog-track-event,"Developer")]/text()',first=True)
        #     pub = s.xpath('//div[@class="details__content table__row-content"]/a[contains(@gog-track-event,"Publisher")]/text()',first=True)
        # return [id,title,r.url,genre,desc,dev,pub]
    except Exception:
...
def gog(u_list=None, save_path:str='', chunk_len:int = 0, max_workers:int = 0, lag:float = 0):
    global flag
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        t2.join()
        print('保存中。。。')
        append_data_xlsx(d_list,save_path,'Sheet1')
        system('cls')
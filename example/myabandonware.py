...
def parse_data(p):
    if not p:
        return None
    r = p
    title,genre,sys,dev,pub,date,score,votes,play,size=['']*10
    try:
        if r:
            s = r.html
        if s:
            ...
            if j:
                ...
                score = (float(score)-1)/4 if score else ''
                votes = int(tmp.get('ratingCount')) if tmp else ''
        return [title,r.url,genre,sys,play,dev,pub,date,size,score,votes]
    except Exception:
...
def myabandonware(u_list=None, save_path:str='', chunk_len:int = 0, max_workers:int = 0, lag:float = 0):
    global flag

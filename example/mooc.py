...
# 获取课程数据
def get_course_data(sql, page, category, category_name):
    querystring = {"csrfKey": "22ce62ac935b4cf99c75e5ba0cdd4524"};
    ...
    if res_json["result"]:
        for course in res_json["result"]:
            save_to_mysql(sql, course, category_name)
...
# 存储到数据库
def save_to_mysql(sql, data, category_name):
    sql.append(['https://www.icourse163.org/course/' + str(data["schoolPanel"]["shortName"]) + "-" + str(data["id"]),
               data['learnerCount'],
               data['schoolPanel']['name'],
        ])
    print(sql[-1])
...
def mooc():
    ...
    for item in front_category_id:
        index = 1
        while True:
            more = get_course_data(sql, index, item["id"], item["category"])
            if not more:
                break
            index += 1
    append_data_txt(sql)
